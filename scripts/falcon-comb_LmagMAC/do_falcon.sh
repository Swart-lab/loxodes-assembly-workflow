...
# for Lmag MAC PacBio reads
# Run this in folder: 
# /ebio/abt2_projects/ag-swart-loxodes/assembly/falcon-comb_LmagMAC

ENV=/ebio/abt2_projects/ag-swart-loxodes/envs/pb
THREADS=16
WD=/ebio/abt2_projects/ag-swart-loxodes/assembly/falcon-comb_LmagMAC

case "$1" in
  "reformat_fasta")
    # Reformat Fastq read files into Fasta format
    # Required as input for Falcon (listed in LmagMAC.fofn file)
    source activate bbmap
    mkdir -p reads_unzip
    touch reads_unzip/DO_NOT_BACKUP_THIS_FOLDER
    for i in /ebio/abt2_projects/ag-swart-loxodes/data/pb/{4777_A_HIFI.fastq.gz,4916_A_4777_A3_run538_CCS.fastq.gz,4916_A_HIFI.fastq.gz}; do
      echo $i
      FN=$(basename $i)
  ...
  "unzip_fastq")
    # Concatenate Fastq files for map_polish step
    mkdir -p reads_fastq
    touch reads_fastq/DO_NOT_BACKUP_THIS_FOLDER
    source activate bbmap
    for i in /ebio/abt2_projects/ag-swart-loxodes/data/pb/{4777_A_HIFI.fastq.gz,4916_A_4777_A3_run538_CCS.fastq.gz,4916_A_HIFI.fastq.gz}; do
      echo $i
      pigz -p 8 -cd $i >> reads_fastq/LmagMAC.hifi.fastq
    done
    ;;

  "index_fastq")
    # Index Fastq files for map_polish step
    source activate bio-env
    samtools fqidx reads_fastq/LmagMAC.hifi.fastq
    ;;
  ...
  "map_polish")
    ...
    FQ=reads_fastq/LmagMAC.hifi.fastq
    ALN=racon_polish_ccs/LmagMAC_p_ctg.ccs.pbmm2.bam
    ALN2=racon_polish_ccs/LmagMAC_p_ctg.ccs.pbmm2.filter.sam
    POLISH_ASM=racon_polish_ccs/LmagMAC_p_ctg.racon_polish.fasta
